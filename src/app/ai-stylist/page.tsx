    ...
    <div className="min-h-screen pt-24 pb-32">
      <div className="container mx-auto px-4 md:px-8">
        <div className="max-w-4xl mx-auto text-center mb-16 space-y-4">
          ...
          <h1 className="text-4xl md:text-5xl font-headline font-bold">Your Personal Atelier Stylist</h1>
          <p className="text-muted-foreground text-lg font-body">
            Describe your needs, and let our intelligence find the perfect handbag companion for your style.
          </p>
        </div>
          ...
          <div className="lg:col-span-5">
            <Card className="rounded-none border-primary/20 shadow-xl overflow-hidden">
              <div className="bg-primary p-6 text-primary-foreground">
                <h3 className="font-headline tracking-widest font-bold">Preferences</h3>
              </div>
              <CardContent className="p-8">
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="space-y-2">
                    <Label htmlFor="occasion" className="text-xs tracking-widest uppercase font-bold text-muted-foreground">The Occasion</Label>
                    <Input id="occasion" name="occasion" placeholder="e.g., Summer Wedding, Board Meeting" required className="rounded-none border-muted focus:border-primary" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="stylePreference" className="text-xs tracking-widest uppercase font-bold text-muted-foreground">Style Preference</Label>
                    <Input id="stylePreference" name="stylePreference" placeholder="e.g., Minimalist, Bold & Trendy" required className="rounded-none border-muted focus:border-primary" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="wardrobeItems" className="text-xs tracking-widest uppercase font-bold text-muted-foreground">Matching Wardrobe</Label>
                    <Textarea id="wardrobeItems" name="wardrobeItems" placeholder="What are you planning to wear? (e.g., Silk dress, tailored suit)" required className="rounded-none border-muted focus:border-primary min-h-[100px]" />
                  </div>
                  <Button type="submit" disabled={loading} className="w-full bg-primary text-primary-foreground hover:bg-primary/90 h-14 rounded-none tracking-widest uppercase font-bold">
                    {loading ? <Loader2 className="animate-spin h-5 w-5 mr-2" /> : "Get Recommendations"}
                  </Button>
                </form>
              </CardContent>
            </Card>
          ...
          <div className="lg:col-span-7">
            {loading ? (
              <div className="h-[500px] flex flex-col items-center justify-center text-center space-y-6">
                ...
                <p className="text-lg font-headline italic text-muted-foreground">Consulting our archives for your perfect match...</p>
              </div>
            ) : result ? (
              <div className="space-y-8 animate-fadeUp">
                <div className="bg-white border-l-4 border-primary p-8 shadow-sm">
                  <h2 className="text-2xl font-headline font-bold mb-4">Stylist's Note</h2>
              ...
              <div className="h-[500px] border-2 border-dashed border-muted rounded-none flex flex-col items-center justify-center text-center p-12 space-y-4 bg-muted/20">
                ...
                <h3 className="text-2xl font-headline font-bold text-muted-foreground">Your Recommendations Will Appear Here</h3>
                <p className="text-muted-foreground max-w-sm">Complete the form to receive a curated selection of handbags tailored specifically to your upcoming event and wardrobe.</p>
              </div>
            )}
          </div>
        </div>
